...
`poly-position-watcher` 面向 Polymarket 实盘交易场景。它要解决的核心问题不是“能不能看到仓位”，而是订单成交、trade 更新、仓位同步、链上可卖余额这几个状态，经常不是同一时刻到达。

在真实策略里，`order filled`、`trade confirmed`、`position updated`、`sellable on-chain` 是不同层级的状态。如果策略只盯住其中一个信号，就很容易高估仓位、过早对冲、过早平仓，或者在状态未完全同步时做出错误判断。

这个库更像一个 Polymarket 策略的“状态一致性 / 执行可靠性组件”。它把这些分散的状态统一观察与整理，让策略代码不必把“订单成交了”误认为“仓位已经同步完成”，也不必把“仓位能看到了”误认为“现在就真的可卖”。
...
- 用于成交判断的仓位字段：
  `size`（净仓位 shares）、`original_size`（原始成交 shares）、`sellable_size`（链上 CONFIRMED 可卖出仓位）、`fee_amount`（累计 USDC 手续费）
...
with PositionWatcherService(
    ...
    print(order)
    if position:
        print("size(净 shares):", position.size)
        print("original_size(原始 shares):", position.original_size)
...
- 返回仓位字段语义：
  `size` = 净仓位 shares，`original_size` = 原始成交 shares，`fee_amount` = 累计 USDC 手续费

默认手续费公式（未传 `fee_calc_fn` 时）：
`fee = size * rate * price * (1 - price)`。

根据当前 Polymarket 官方文档，taker fee 以 USDC 计费。
默认实现下，无论 taker buy 还是 taker sell，都不会减少 shares 数量；手续费影响只体现在 `fee_amount`，不会通过扣减 `size` 表达。
